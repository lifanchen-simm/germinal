...
class _af_prep:
  ...
  def _prep_binder(self, pdb_filename,
    ...
    -bias_redesign [float or False]: redesign binder by exclusively setting the bias to specific value.
    ...
    -sequence: sequence to use as initial sequence. Only useful when using a template with diff. sequence
    ---------------------------------------------------
    '''
    redesign = binder_chain is not None
    rm_binder = not kwargs.pop("use_binder_template", not rm_binder)
    self.binder_redesign = redesign
    self.use_pos_distance = use_pos_distance
    self.dimer = dimer

    if bias_redesign is None: bias_redesign = False
    ...
    ### #produces a dict with only info on the positions selected
    if pos is not None:
      pos_set = []
      chain_info = [binder_chain.split(",")[0]] * len(pos)
      residue_pos = np.array(pos) + 1
      for i,c in zip(residue_pos, chain_info):
        idx = np.where((self._pdb["idx"]["residue"] == i) & (self._pdb["idx"]["chain"] == c))[0]
        assert len(idx) == 1, f'ERROR: positions {i} and chain {c} not found'
        pos_set.append(idx[0])

      self.opt["pos"] = self._pdb["pos"] = np.array(pos_set)
      
    # configure input features
    #prepare empty features of size target+binder (eg. 150 + 205)
    ...
    if hotspot is not None:
      ...
      if dimer:
        ...
        hotspot_dimer = hotspot.replace(target_chains[0], "X").replace(target_chains[1], target_chains[0]).replace("X", target_chains[1])
        self.opt["hotspot_dimer"] = prep_pos(hotspot_dimer, **self._pdb["idx"])["pos"]

    if redesign:
      self._wt_aatype = self._pdb["batch"]["aatype"][self._target_len:]
      self.opt["weights"].update({"dgram_cce":1.0, "rmsd":0.0, "fape":0.0,
                                  "con":1.0, "i_con":1.0, "i_pae":1.0})
    else:
    ...
    if redesign:
      if sequence is None:
        kwargs['seq'] = jax.nn.one_hot(self._pdb['batch']['aatype'][self._target_len:],self._args["alphabet_size"])
      else:
        kwargs['seq'] = sequence
        kwargs['set_wt'] = True
